...
layout(rgba32f, binding = 0) uniform image3D img_dist_field;
...
void main() {
  ...
  float minimum = r_probe; //NOTE: 2 * r_probe can be a problem for raymarching, we actually need the default value to be the probe radius!
  ...
  imageStore(img_dist_field, ivec3(pixel_coords), pixel);
  // Last argument has to be vec4 even though texture has only red component
  imageStore(img_b_factor, ivec3(pixel_coords),
             vec4(closest.b_factor, 0, 0, 0));
}