...
vec3 calculate_normal(vec3 coords_grid) {
  ...
  float gradient_x = texture(grid, coords_grid + epsilon_vec.xyy).a -
                     texture(grid, coords_grid - epsilon_vec.xyy).a;
  float gradient_y = texture(grid, coords_grid + epsilon_vec.yxy).a -
                     texture(grid, coords_grid - epsilon_vec.yxy).a;
  float gradient_z = texture(grid, coords_grid + epsilon_vec.yyx).a -
                     texture(grid, coords_grid - epsilon_vec.yyx).a;
...
vec3 sobel_normal(vec3 coords_grid) {
  ...
  float xxx = texture(grid, coords_grid + e.xxx).a;
  float xxy = texture(grid, coords_grid + e.xxy).a;
  float xxz = texture(grid, coords_grid + e.xxz).a;
  float xyx = texture(grid, coords_grid + e.xyx).a;
  float xzx = texture(grid, coords_grid + e.xzx).a;
  float xyy = texture(grid, coords_grid + e.xyy).a;
  float xyz = texture(grid, coords_grid + e.xyz).a;
  float xzy = texture(grid, coords_grid + e.xzy).a;
  float xzz = texture(grid, coords_grid + e.xzz).a;

  float yxx = texture(grid, coords_grid + e.yxx).a;
  float yxy = texture(grid, coords_grid + e.yxy).a;
  float yxz = texture(grid, coords_grid + e.yxz).a;
  float yyx = texture(grid, coords_grid + e.yyx).a;
  float yyz = texture(grid, coords_grid + e.yyz).a;
  float yyy = texture(grid, coords_grid + e.yyy).a;
  float yzx = texture(grid, coords_grid + e.yzx).a;
  float yzy = texture(grid, coords_grid + e.yzy).a;
  float yzz = texture(grid, coords_grid + e.yzz).a;

  float zxx = texture(grid, coords_grid + e.zxx).a;
  float zxy = texture(grid, coords_grid + e.zxy).a;
  float zxz = texture(grid, coords_grid + e.zxz).a;
  float zyx = texture(grid, coords_grid + e.zyx).a;
  float zyy = texture(grid, coords_grid + e.zyy).a;
  float zyz = texture(grid, coords_grid + e.zyz).a;
  float zzx = texture(grid, coords_grid + e.zzx).a;
  float zzy = texture(grid, coords_grid + e.zzy).a;
  float zzz = texture(grid, coords_grid + e.zzz).a;
...
void main() {
  ...
  for (i = 0; i < max_steps; i++) {
    ...
    vec4 result = texture(grid, coords_grid);
    dist = result.a;
    color = result.xyz;
  ...
  if (i < max_steps) {
    ...
    // normal = sobel_normal(coords_grid);
  } else {
    normal = vec3(0.0);
  }
