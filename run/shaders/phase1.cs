...
//layout(r32f, binding = 4) uniform image3D img_b_factor;
...
uniform vec3 n_dims; // x,y, and z size of the neighborhood grid
...
void main() {
  ...
  vec4 pixel;
  vec4 result = vec4(closest.element_color, minimum);
  pixel.xyz = result.xyz;

  // Calculate the global index of the work group. This is used to flag blocks
  // of atoms in the boundary region for further refinement
  uint global_index =
      gl_WorkGroupID.z * gl_NumWorkGroups.x * gl_NumWorkGroups.y +
      gl_WorkGroupID.y * gl_NumWorkGroups.x + gl_WorkGroupID.x;

  //  point is outside of the SES
  if (result.a > r_probe) {
    pixel.a = r_probe;
    pixel.xyz = vec3(0);
  }

  //  point is in the boundary region
  else if (result.a > -resolution) {
    pixel.a = resolution;
    atomicExchange(index[global_index], global_index);
  }

  //  point is inside the SES
  else {
    pixel.a = -resolution;
  }

  imageStore(img_dist_field, ivec3(pixel_coords), vec4(pixel.a, 0.0, 0.0, 0.0));
}