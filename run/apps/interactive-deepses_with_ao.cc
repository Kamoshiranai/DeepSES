...
//          deviceProperties.maxpatchesPerDim[0],
//          deviceProperties.maxpatchesPerDim[1],
//          deviceProperties.maxpatchesPerDim[2]);

//   printf("---------------------------------------\n");
// }

// bool isMatrixValid(const float4x4& mat) {
//   for (int i = 0; i < 4; ++i) {
//       if (std::isnan(mat.rows[i].x) || std::isinf(mat.rows[i].x) ||
//           std::isnan(mat.rows[i].y) || std::isinf(mat.rows[i].y) ||
//           std::isnan(mat.rows[i].z) || std::isinf(mat.rows[i].z) ||
//           std::isnan(mat.rows[i].w) || std::isinf(mat.rows[i].w)) {
//           fprintf(stderr, "!!! Matrix row %d contains NaN/Inf !!!\n", i);
//           return false;
//       }
//   }
//   return true;
...
// void copyAndPrintBuffer(cudaPitchedPtr d_batchedInputBuffer, int batch, int patchSize) {
//   size_t pitch = d_batchedInputBuffer.pitch;
//   size_t rowSize = patchSize * sizeof(float);
//   size_t height = patchSize;
//   size_t depth = batch * patchSize;

//   float* h_buffer = new float[batch * patchSize * patchSize * patchSize];

//   // Copy row by row
//   for (int z = 0; z < depth; ++z) {
//       for (int y = 0; y < height; ++y) {
//           cudaMemcpy(h_buffer + z * patchSize * patchSize + y * patchSize, 
...
glm::vec3 uniform_color = glm::vec3(26, 41, 88) / 100.0f; // blue
...
int main(int argc, char *argv[]) {
  ...
  // after set up unmap resources -> can be used by opengl again
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // Display FPS (e.g., in window title or using ImGui)
    ...
    // camera.rotate_angle_axis(glm::radians(0.5), glm::vec3(0, 1, 0)); //NOTE: 10.0 radians for avg. over 36 frames
    camera_changed = true;
    ...
    if (camera_changed) {
      ...
      // Record START event for CUDA operations
        ...
        dim3 blocksRaymarchKernel((SCR_WIDTH / 8 + threadsRaymarchKernel.x - 1) / threadsRaymarchKernel.x,
                      (SCR_HEIGHT / 8 + threadsRaymarchKernel.y - 1) / threadsRaymarchKernel.y); //NOTE take only every second/4th pixel or so
        ...
        dim3 threadsPerBlock(512); //NOTE: tune for your gpu

        if (selected_count == 0) {
            std::cout << "No patches selected. Skipping gather, processing, and scatter." << std::endl;
        } else {
          dim3 blocksForGather(batchDim, 1, 1); // Iterate over original patches
        ...
        for (int batch = 0; batch < (selected_count + batchSize - 1) / batchSize; batch++) { // ceil division
          ...
          float* batchOutput = reinterpret_cast<float*>(filteredBatchedInputBuffer) + offset;
  ...
  // Clean up cuda stuff
  cudaGraphicsUnmapResources(1, &cudaInputResource, stream); // Unmap when done
